...
function buildPrompt(fromDate, toDate) {
  return `Today is ${fromDate}. Search the web and return ALL upcoming sports events between ${fromDate} and ${toDate} (inclusive) across all six categories below. Apply every filtering rule exactly as written.
...
━━ CATEGORY 6: LEBANESE BASKETBALL ━━
...
[
  {
    "title": "event title",
    "sport": "football|f1|nba|ufc|tennis|lebanese_basketball",
    "date": "YYYY-MM-DD",
    "time_utc": "HH:MM",
...
export async function fetchClaudeEvents(fromDate, toDate) {
  ...
  for (const model of MODELS) {
    for (const useWebSearch of [true, false]) {
      try {
        ...
        console.error(`[claude] FAILED model=${model} webSearch=${useWebSearch}`)
        console.error(`[claude]   message:  ${err.message}`)
        console.error(`[claude]   status:   ${err.status ?? 'n/a'}`)
        console.error(`[claude]   error:    ${JSON.stringify(err.error ?? null)}`)
        console.error(`[claude]   response: ${JSON.stringify(err.response ?? null)}`)
      }
    }
    if (rawText) break
  }
  ...
  if (!rawText) {
    console.error('[claude] all attempts failed — see errors above for details')
    return []
  }
