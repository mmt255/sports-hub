...
Your entire response must start with [ and end with ]. Return [] if nothing found.
...
async function callClaude(sport, prompt) {
  ...
  try {
    ...
    console.log(`[claude:${sport}] stop_reason=${resp.stop_reason} input_tokens=${resp.usage?.input_tokens} output_tokens=${resp.usage?.output_tokens}`)
...
export async function fetchClaudeEvents(fromDate, toDate) {
  ...
  console.log(`[claude] running ${sports.length} parallel sport calls for ${fromDate}→${toDate}`)

  // Run all sport calls in parallel; individual failures don't block others
  const results = await Promise.allSettled(
    sports.map(({ key, prompt }) =>
      callClaude(key, prompt).then(text => parseJSON(key, text))
    )
  )
  ...
  results.forEach((result, i) => {
    const key = sports[i].key
    if (result.status === 'fulfilled') {
      counts[key] = result.value.length
      allRaw.push(...result.value)
    } else {
      counts[key] = 0
      console.error(`[claude:${key}] promise rejected: ${result.reason}`)
    }
  })
