...
export async function writeCache(data) {
  if (hasRedis()) {
    try {
      const redis = await getRedis()
      console.log('[cache] writing to Redis, events:', data.events?.length ?? 0)
      // No expiry — cron job refreshes daily
      await redis.set(CACHE_KEY, data)
      console.log('[cache] Redis write OK')
      return
    } catch (err) {
      console.error('[cache] Redis write failed:', err.message, '| status:', err.status ?? 'n/a', '| body:', JSON.stringify(err.error ?? null))
    }
  }
